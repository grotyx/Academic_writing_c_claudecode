...
- **Author response DOCX generation**（`scripts/compile_response_docx.py`）— DOCX-ready Markdown を `Author_response_220803_Final.docx` 参照スタイルに変換
...
`Author_response_220803_Final.docx` が存在する場合、compiler はこのファイルを reference style document として使用します。
...
- Constraint、Citation、Data Verifier と gate ledger を導入。
